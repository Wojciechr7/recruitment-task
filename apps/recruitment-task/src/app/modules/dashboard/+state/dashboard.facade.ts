...
export class DashboardFacade {
  ...
  cart$ = this.store.pipe(select(DashboardSelectors.getCart));
  ...
  addToCart(dish: DishModel) {
    this.store.dispatch(DashboardActions.moveDishToCart({ dish }));
  }

  removeFromCart(dish: DishModel) {
    this.store.dispatch(DashboardActions.removeDishFromCart({ dish }));
  }
